...
.stButton>button { width: 100%; border-radius: 8px; height: 3em; background-color: #6C63FF; color: white; font-weight: bold; }
</style>
...
def overlay_logo(frame, logo_bgra, bbox, scale_val):
    x, y, w, h = [int(v) for v in bbox]
    
    # Apply logo scaling (multiplier)
    new_w = int(w * scale_val)
    new_h = int(h * scale_val)
    # Adjust X and Y to keep it centered
    x = x - (new_w - w) // 2
    y = y - (new_h - h) // 2
    
    fh, fw = frame.shape[:2]
    x, y = max(0, x), max(0, y)
    new_w, new_h = min(new_w, fw - x), min(new_h, fh - y)
    
    if new_w <= 10 or new_h <= 10: return frame
    
    try:
        resized = cv2.resize(logo_bgra, (new_w, new_h), interpolation=cv2.INTER_AREA)
        if resized.shape[2] == 4:
            alpha = resized[:, :, 3:4] / 255.0
            frame[y:y+new_h, x:x+new_w] = (resized[:,:,:3] * alpha + frame[y:y+new_h, x:x+new_w] * (1 - alpha)).astype(np.uint8)
        else:
            frame[y:y+new_h, x:x+new_w] = resized[:,:,:3]
    except: pass
    return frame
...
if v_file:
    ...
    if st.button("📱 Convert to Shorts (9:16 Center Crop)"):
        with st.spinner("Reformatting for TikTok/Shorts..."):
            ...
            st.success("Shorts Format Ready!")
    ...
    if ret:
        ...
        if canvas_result.json_data is not None:
            ...
            if len(objs) > 0 and logos:
                st.info(f"Targets detected: {len(objs)}. Assign logos and adjust size:")
                
                logo_scale = st.slider("Logo Size Multiplier", 0.5, 3.0, 1.2)
                ...
                if st.button("🚀 TRACK & RENDER FINAL VIDEO", type="primary"):
                    # Use CSRT Tracker (High Accuracy for non-humans)
                    trackers = []
                    for obj in objs:
                    ...
                    while cap.isOpened():
                        ...
                        for i, tracker in enumerate(trackers):
                            ...
                            if ok:
                                f = overlay_logo(f, logos[mapping[i]], bbox, logo_scale)
                    ...
                    # Merge Audio
                    final_path = "puny_god_final.mp4"
                    with st.spinner("Merging original audio..."):
                        mux_audio(temp_vid.name, st.session_state.shorts_path, final_path)
                    
                    st.success("✅ Complete!")
                    with open(final_path, "rb") as file:
                        st.download_button("⬇️ Download Result", file, "tracked_video.mp4", use_container_width=True)
            elif len(objs) > 0 and not logos:
                st.warning("Please upload your logos first!")
